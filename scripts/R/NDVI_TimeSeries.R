...
p <- ggplot(ndvi_2020_24, aes(x = site_name, y = mean, fill = Species)) + # To create the boxplot
  geom_boxplot(outlier.alpha = 0.5) +
  scale_fill_manual(values = fill_vals, name = "Species",
                    labels = c(expression(italic("C. mopane")),
                               expression(italic("B. plurijuga")))) +
  scale_x_discrete(labels = label_fun) +
  labs(
    title = "NDVI distribution per site (2020-2024)",
    x = "Sites", y = "NDVI"
  ) +
...
stats_per_site <- ndvi_2020_24 %>%     # To get the specifics values of each box
  group_by(site_name, Species, Country) %>%
  summarise(
...
xlsx_path <- file.path(out_dir, "NDVI_stats_per_site_2020_2024.xlsx")
#write_xlsx(list("stats_per_site" = stats_per_site), path = xlsx_path)
